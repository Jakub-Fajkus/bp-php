...
Cílem práce je nalézt metodu řízení počítačového modelu jednoduchého robota, který bude vykonávat
netriviální pohyb. \todo{nebo spise vyvinout framework pro hledani metod obecne?}
%Pro hledání způsobu řízení jsou použity algoritmy simulující evoluci.
Pro řízení modelu robota byly použity dva rozdílné přístupy.
První přístup je založen na instrukcích(inspirováno[nebo rovno] LGP).
Druhý přístup využívá jednoduché neuronové sítě.
Kontrolery z obou pristupu jsou vyvijeny pomoci GA.
\todo{Vysledky?} Tato prace overila, ze lze provadet rizeni modelu robota obemi zpusoby
se zavedenim samoadaptace(tedy environment awareness).
Bylo take ukazano, jak jednoduchy pristup bez samoadaptce selhava v netrivialnich pripadech.
Dale bylo demonstrovano, ze neuronove site
maji lepsi vysledky, nez-li pouzity pristup zalozeny na instrukcich(
\todo{Diskutatibilni, NN jsou jendodussi na environment, protoze je tam aritmetika a vyhodocovani celkove
blackbox a neni treba vymyslet instrukce a vazbu na interpret})
Hlavnim prinosem prace je overeni konceptu rizeni modelu robota a srovnani dvou rozdilnych pristupu
k tomuto problemu.


Cilem teto prace je vytvorit framework, ktery bude pouzit pro hledani zpusobu rizeni pocitacoveho
modelu jednoducheho robota. Tento model bude v pocitacove simulaci vykonavat netrivialni pohyb.
Pro řízení modelu robota jsou použity dva rozdílné přístupy.
První přístup je založen na instrukcích(inspirováno \todo{nebo rovno} LGP.
Druhý přístup využívá jednoduché neuronové sítě.
Kontrolery z obou pristupu jsou vyvijeny pomoci GA.
\todo{Vysledky?} Tato prace overila, ze lze provadet rizeni modelu robota obema zpusoby
se zavedenim samoadaptace(tedy environment awareness).
Bylo take ukazano, jak jednoduchy pristup bez samoadaptce selhava v netrivialnich pripadech.
Dale bylo demonstrovano, ze neuronove site maji lepsi vysledky, nez-li pouzity pristup zalozeny
na instrukcich(\todo{Diskutatibilni, NN jsou jendodussi na environment, protoze je tam aritmetika
a vyhodocovani celkove blackbox a neni treba vymyslet instrukce a vazbu na interpret})
Hlavnim prinosem prace je overeni konceptu rizeni modelu robota a srovnani dvou rozdilnych pristupu
k tomuto problemu.
...
\todo{asi bych prevzal uvod do nejake robotiky obecne}
...
Mame tedy modely jednoduchych robotu, castecne inspirovane prirodou. \todo{viz obr.}
Cilem prace je nalezt program nebo konfiguraci neuronove site tak, aby model robota vykonaval dany netrivialni pohyb.
...
Algoritmy pomoci evoluce vyvijeji program, nebo neuronovou sit tak, aby se model pohyboval po pozadovane trajektorii.
%Vysledne reseni by tedy melo ridit model robota takovym zpusobem, ze se jeho trajektorie bude co nejvice blizit trajektorii nami pozadovane.
...
\begin{description}[noitemsep]
	\item[2018-ExcelFIT-ShortName.tex] This is the template for the main \LaTeX{} file -- this is your paper.  Do yourself a favor and replace \textit{ShortName} in the filename with something meaningful.
	\item[2018-ExcelFIT-ShortName-bib.bib] You can delete the contents of this file completely and start adding BibTeX references.  It is much easier to use a small editing tool (Section~\ref{sec:UsefulTools}, JabRef) than to format \textit{.bib} file manually.  Rename the file so that \textit{ShortName} is consistent with the previous file (and update the filename in the \textit{.tex} file).
...
\begin{description}[noitemsep]
\item[.pdf] Perfect for vector graphics.  All graphs \textbf{must} be in vector and therefore in .pdf.  Gnuplot, pyplot, Matlab -- they all produce vector graphs in .pdf easily.  Diagrams, system structures, sketches -- all vector graphics.  It's 2018, not 1980 anymore\ldots
...
\begin{description}
	\item[\href{http://www.overleaf.com/}{Overleaf}] Online \LaTeX{} editing -- if you don't want to install and learn many tools, Overleaf is a great solution: works online and allows sharing your text with your supervisor.
