...
%Tato sekce o LGP by měla vycházet čistě z lit., na 1-2 strany popište základní myšlenky LGP a doplňte ilustracemi tak,
%aby bylo zřejmé, jak se programy interpretují a jak je možné je navrhovat evolučně. Např. takto:
%
%Lineární genetické programování (LGP) je speciálním případem genetického programování, popsaného v sekci XX,
%kdy jsou programy reprezentovány jako lineární struktury/seznamy příkazů (instrukcí) vhodného imperativního
%programovacího jazyka. Příklad struktury programu v LGP ukazuje obrázek XX (zde uvést jednodychý program např.
%realizující cyklus pro sečtení pole čísel).
%Obecně jsou data, zpracovávaná pomocí programu LGP, uchovávána v registrech, které jsou součástí interpretu LGP.
%Každý program zpracovává určitá vstupní data, využívá interní proměnné a konstanty a produkuje výstupní data.
%Pro každou tuto kategorii dat je v interpretu LGP implementována sada registrů, které jsou podle účelu nazývány
%vstupními registry, výstupními registry, konstantami či pracovními registry obsahující proměnné.
%V dalším odstavci popište podrobně vykonání příkladu programu (krok za krokem), kde také vysvětlete, jak jsou
%inicializovány a modifikovány jednotlivé registry a kde se potom vezme výsledek
