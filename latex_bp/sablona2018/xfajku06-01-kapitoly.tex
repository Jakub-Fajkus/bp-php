...
\todo{vlna!}
\todo{dat obrazky vedle sebe, asy se usetrilo misto!}
...
V této sekci shrneme myšlenky vycházející z relevantní literatury a představíme vlastní koncept evolučního návrhu řízení robotů.

\todo{jak citovat obrazky?}

\section{Návrh pohyblivých struktur pomocí L-systémů}
G. Hornby ve své disertaci pomocí generativních reprezentací (L-systémů) mimo jiné vyvíjel fyzickou strukturu i řízení robotických modelů.
...
G. Hornby touto metodou vyvinul množstí robotů (obrázek ~\ref{fig:hornby_roboti_1}), z nichž některé i skutečně vyrobil, viz obrázek~\ref{fig:hornby_roboti}
...
%V odstavečcích cca 5-10 řádků uveďte v sekci 3.1:
%- Hornby se zabýval využitím L-systémů jako generativní reprezentace pro řízení robotů, popovídat, jak to dělal s uvedením
%jednoduché ilustrace, jak jeho řízení funguje a jaký použil EA.
%- V dalším odstavci opět popsat, na jaké typy pohyblivých struktur se Hornby zaměřil, co se mu podařilo úspěšně navrhnout a opět
%odkázat na obr. ukazující vybrané 2-3 výstupy (včetně náhledu na fyzickou realizaci - viz str. 108 hornbyho disertace).


\section{Wolff a Wahde!!!!}
Wolff a Wahde ve své práci využili koncept LGP pro vývoj kontroléru pro humanidního robota, který měl za úkol chodit v simulovaném porstředí.
Použili zde 7 instrukcí: sčítání, odčítání, násobení, dělění, funkce sinus, a dvě varianty větvení.
